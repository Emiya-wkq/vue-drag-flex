<template>
  <div class="item-box" :style="{'width': width,'height': height}" ref="itemElement">
    ...
    <div class="slider slider-row" v-if="dragLayout.mode==='row'" @mousedown="mousedown" ref="slider">
      ...
      <div v-if="!$slots.slider" class="default-row-slider">
      </div>
    </div>
    <div class="slider slider-columns" v-if="dragLayout.mode==='column'"  @mousedown="mousedown" ref="slider">
      <slot name="slider"></slot>
      <div  v-if="!$slots.slider"  class="default-columns-slider">
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods:{
      ...
      mousedown(e){
        let element =  this.$refs['itemElement']
        this.x=e.x
        this.y=e.y
        this.originWidth = element.clientWidth
        this.originHeight = element.clientHeight
...
<style scoped>
  ...
  /* 默认拖动栏样式 */
  .default-columns-slider{
    display: flex;
    justify-content: center;
  }
  .default-columns-slider::before{
    content: '—';
    height: 8px;
    width: calc(100% - 40px);
    border-radius: 3px;
  ...
  .default-columns-slider:hover:before{
    opacity: 1;
  }
  .default-row-slider{
    display: flex;
  ...
  .default-row-slider:hover:before{
    opacity: 1;
  }
</style>
